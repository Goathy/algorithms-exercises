...
class Node {
	constructor(string) {
		this.children = []
		// indicate if node add up to a valid word
		this.terminus = false
		this.value = string[0] || ""
	...
	complete(string) {
		const completions = []
		for (const child of this.children) {
			child.#complete(string, "", completions)
		}

		return completions
	}

	#complete(search, base, suggestions) {
		...
		if (this.terminus) {
			suggestions.push(`${base}${this.value}`)
		}

		for (const child of this.children) {
			child.#complete(search.substr(1), `${base}${this.value}`, suggestions)
		}
